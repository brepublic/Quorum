...
- 当前启动仅发布 `builtin:beijing-academic` 内置规则的 v9，移除“讨论决议草案”、两项延长磋商、关闭有主持核心磋商及暂停决议草案发言名单动议。普通有主持核心磋商和单独的草案关联磋商选项仍提交同一种动议；后者以草案 ID 关联，议题默认按委员会内容语言生成，初始发言队列为空。有主持核心磋商不能由名单状态直接关闭或重开；有主持和自由磋商计时器不能延长，进行中的自由磋商也不能重置。本机开发清理脚本只保留 v9，删除引用其他版本的委员会及旧规则版本；正常启动不执行清理。
