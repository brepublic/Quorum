...
动议、问题、国家和席位的搜索词由服务器本地 `pinyin-pro`、英文缩写规则及内置国家二字码生成，保存在 PostgreSQL 的可重建索引中；浏览器只匹配服务器返回的候选项，不请求外部拼音服务。索引在首次部署和生成规则版本变化时整体重建，名称变化时按来源校验并更新受影响的词；系统管理员可从运行状态页手动整体重建。账号维护的国家检索代码单独存放，重建不会覆盖。内置国家的手填代码仅供填入账号本人使用；自定义国家模板的手填代码也供使用该模板的委员会用户和代表文件入口使用。搜索词不进入委员会历史内容快照。
...
`deploy/compose.yaml` 为当前生产拓扑。Caddy 代理 `/api/v1/*`、`/health/*` 和 `/metrics`，其余路径回退 SPA。容量默认 80% warning、90% critical；critical 只拒绝新文件字节和 provider copy，下载、议事与清理继续可用。Compose 的 JSON 日志固定轮换为 10 MiB × 3。CI 在前端、服务端和 PostgreSQL 测试通过后，以 `linux/amd64` 构建应用和 Caddy 镜像，再用独立的 `deploy/compose.release-smoke.yaml`、临时数据库与文件卷检查迁移、HTTPS、就绪状态、版本、匿名身份入口和 SPA 路由；此检查不发布镜像。
...
pnpm test:self-host:frontend       # src/ 下的前端测试
pnpm test:self-host:server         # 服务端、共享包及 scripts/ 的非 PostgreSQL 测试
...
集成测试必须使用 `TEST_DATABASE_ADMIN_URL` 创建随机临时数据库；未配置时明确 skip，不使用内存数据库替代。GitHub Actions 并行运行前端、服务端与共享代码、PostgreSQL 三个 job；各 job 独立构建，只有数据库 job 启动 PostgreSQL 16 service。服务端 job 另检查生产运行时。真实 PostgreSQL、浏览器、TLS、S3 和原生平台证据仍不能由普通测试替代。
