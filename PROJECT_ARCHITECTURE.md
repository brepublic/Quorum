...
点名页使用每页 9 席的三列三行矩阵，按列从上到下排列。当前内置规则只提供“出席”和“缺席”；点名只记录出席状态。使用委员会模板时，模板给出席位的初始表决权；席位设置页负责之后的调整和手动新增席位的设置。Chair 可直接选择冻结名单中的任意席位并改答；浏览器仍只提交 `baseRevision`、目标席位和冻结回答，服务端在事务内追加新的 entry、撤销被替代 entry，并生成事件、审计及完成后的出席事件。原有顺序录入、撤销和重置命令继续保留。
...
- 初始化补齐同名 `builtin:beijing-academic` 第 6 版，新建委员会默认选用它；不重建旧规则。显式的本机开发清理脚本 `server/scripts/consolidate-development-rules.mjs --local-development-rebuild` 保留第 6 版，删除其他规则版本及采用其他版本的委员会，不在正常启动或迁移中执行破坏性清理。
