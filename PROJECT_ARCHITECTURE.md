...
> Quorum 已完成自主托管目标设计，并已落地实施计划阶段 0–8。`self-hosted` 运行时以 PostgreSQL 为唯一业务真相，通过同源 API 与受众过滤 SSE 提供议事功能；上传可流式暂存并提交到服务器持久卷、S3 compatible provider 或当前主席电脑。文件可在自托管工作区审核、发布、授权下载、异步物理删除并在服务器卷与 S3 之间安全迁移。委员会可进入全工作区只读归档，由 Owner 流式导出记录，或经精确名称确认进入 durable 永久删除；删除 worker 等待 provider、Agent 和全部暂存副本清理后，才在单个事务中清除委员会业务数据。禁用账号可在资源原子转移后不可逆匿名化，历史议事与审计继续引用无个人信息的稳定用户 ID。常驻保留 worker 只清理明确到期的临时身份数据，事件、审计和 durable task 不被普通期限删除。系统管理员可查看固定聚合运行状态；仓库提供 PostgreSQL dump、文件 manifest 和隔离恢复演练说明，但不调度自动备份。桌面 Agent 已具备安全目录、墓碑优先同步、原子落盘、全量扫描、中断恢复和显式冲突裁决，并可生成携带固定运行时的 Windows、macOS 与 Linux 发布包。原生签名、公证、安装和恢复证据仍须在目标系统取得。
...
- 阶段 8.4 启动带 PostgreSQL advisory lock 的常驻 retention worker：过期/已撤销 Session 保留 30 天，身份命令幂等结果保留 30 天，一次性邀请/配对秘密终态保留 7 天，已决定注册申请保留 90 天；业务幂等结果仍按自身 `expires_at` 删除。每次 sweep 在一个事务内提交并写追加式运行证据，失败整体回滚并只记录稳定错误码。委员会事件、业务/身份审计、Agent task、文件删除 job 和 deletion job 不参与普通期限清理；结构化 stdout 日志由 Compose 的 10 MiB × 3 文件上限轮换。
- 阶段 8.5 增加仅系统管理员可读的运行状态 API 与页面，聚合 schema compatibility、存储容量状态、账号/委员会数量、固定队列深度和最近 retention 结果，不返回名称、文件路径、provider key 或凭据。`self-host:backup` 使用 `pg_dump` custom format 输出数据库 dump，并从 PostgreSQL 文件元数据生成内部 provider manifest 和双文件 SHA-256 元数据；恢复必须在隔离实例核对 manifest，当前不提供自动备份计划或自动破坏性 restore。
...
| `packages/storage-agent/` | Chair Local Agent 的 HTTPS 客户端、安全目录、原子文件操作、扫描、恢复循环与发布包安装说明 |
