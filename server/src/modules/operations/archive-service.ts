...
export const ARCHIVE_SECTIONS: readonly ArchiveSection[] = Object.freeze([
  ...
  {name: 'meeting_sessions', query: `SELECT id,ordinal,phase_id,active_rule_package_version_id,status,revision,created_at,closed_at FROM meeting_sessions WHERE committee_id=$1 ORDER BY created_at,id`},
  ...
  {name: 'resolutions', query: `SELECT r.* FROM resolutions r JOIN documents d ON d.id=r.document_id WHERE d.committee_id=$1 ORDER BY r.document_id`},
  {name: 'resolution_countries', query: `SELECT c.* FROM resolution_countries c JOIN documents d ON d.id=c.resolution_document_id WHERE d.committee_id=$1 ORDER BY c.resolution_document_id,c.role,c.seat_id`},
