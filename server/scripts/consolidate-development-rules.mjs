...
try {
  ...
  const keep = (await client.query(`SELECT v.id,v.package_id FROM rule_package_versions v JOIN rule_packages p ON p.id=v.package_id
    WHERE p.scope='BUILTIN' AND p.stable_key='builtin:beijing-academic' AND v.version=8 AND v.status='PUBLISHED'`)).rows;
  if (keep.length !== 1) throw new Error('Expected exactly one published Beijing version 8.');
