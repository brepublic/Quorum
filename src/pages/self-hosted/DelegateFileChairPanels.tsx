...
export function DelegateFileUploadPanel({snapshot, api}: {snapshot: CommitteeWorkspaceSnapshot; api: SelfHostedApi}) {
  ...
  return <div className="delegate-file-upload-panel">
    ...
    <Card centered fluid className="delegate-file-chair-upload"><Card.Content><Form onSubmit={() => void upload()}>
      <Form.Input type="file" label={t("Choose file")} input={{onChange: (event: React.ChangeEvent<HTMLInputElement>) =>
        setSelected(event.currentTarget.files?.[0]), 'aria-label': t("Choose file")}} />
      {progress === undefined ? <Button primary disabled={!selected || working}>{t("Upload files")} <Icon name="arrow up" /></Button>
  ...
  return <div className="delegate-file-review-panel">
    ...
    <div className="delegate-file-card-list">{reviewedFiles.length ? reviewedFiles.map(file => <Card fluid key={file.id} className="delegate-file-card motion-card">
      <Card.Content><div className="motion-heading delegate-file-heading"><Card.Header>{file.logicalName}</Card.Header>
        <span className={`motion-decision ${file.status === 'REJECTED' ? 'motion-decision-failed' : 'motion-decision-passed'}`}>{file.status === 'REJECTED' ? t("File status: Rejected") : t("Approved point")}</span></div>
        <Card.Meta><Table compact celled className="motion-metadata-table delegate-file-metadata"><Table.Body>
          <Table.Row><Table.Cell className="motion-metadata-key">{t("Submitted by")}</Table.Cell><Table.Cell>{file.submissionSource === 'CHAIR' ? t('Chair') : file.submitterDisplayName ?? '—'}</Table.Cell></Table.Row>
          ...
          <Table.Row><Table.Cell className="motion-metadata-key">{t("Submitted at")}</Table.Cell><Table.Cell>{dateTime(file.submittedAt)}</Table.Cell></Table.Row>
          <Table.Row><Table.Cell className="motion-metadata-key">{t("Reviewed at")}</Table.Cell><Table.Cell>{dateTime(file.reviewedAt ?? file.publishedAt)}</Table.Cell></Table.Row>
          <Table.Row><Table.Cell className="motion-metadata-key">{t("Original file")}</Table.Cell><Table.Cell>{file.originalName}</Table.Cell></Table.Row>
          {file.rejectionReason && <Table.Row><Table.Cell className="motion-metadata-key">{t("Rejection reason")}</Table.Cell><Table.Cell>{file.rejectionReason}</Table.Cell></Table.Row>}
        </Table.Body></Table></Card.Meta>
      </Card.Content>{file.status === 'REJECTED' && !file.deleted && <Card.Content extra><Button negative fluid disabled={working}
        onClick={() => setDeleting(file)}><Icon name="times" />  {t("Delete file")}</Button></Card.Content>}</Card>) : <Message content={t("No reviewed files")} />}</div>
