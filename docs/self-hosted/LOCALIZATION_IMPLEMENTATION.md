...
## 2026-09-18 完整数据库回归与切换准备（未部署）

旧集成夹具改为显式创建目录、传入语言/来源 revision/规则版本/幂等键，删除自由席位名与自动名称列写入；预期权限和操作模式显式设置。完整 PostgreSQL 回归 9 文件 119 项通过。另增历史表决读取原版本、归档包含未启用 FUTURE 版本，两项定向通过。构建通过；非数据库 640 项中 639 项通过，唯一失败为文件后缀输入的旧 DOM ID，更新查询后相关 15 项全部通过。

登录/初始化/改密、委员会创建、国家名称编辑和驳回设置接入字段定位；错误标红、说明、展开并聚焦，语言切换不重复抢焦点。未知错误仅展示安全通用文本与受约束 requestId。字段组件和身份/创建页等 24 项通过。

开发切换脚本在 schema 55 临时数据库运行成功，删除委员会业务记录并验证账号、源模板/成员、全局规则、系统配置保持，再升级到最新迁移。保留数据的比较在事务内完成。尚需目标实例切换、浏览器四组合及最终原生验证；这条记录不代表全计划完成。

### 目标库切换发现的删除历史遗漏

第一次目标切换被 `resolution_direct_votes` 外键阻止，整个清理事务回滚。补齐动议直接票、决议直接票、意向性投票修订及文档结果记录的删除顺序，先解除讨论名单的决议引用。migration 63 为两个后加入的 append-only 历史触发器补上现有受控删除例外；普通 UPDATE/DELETE 仍被禁止。schema 55 切换脚本在同一事务预先安装这两个函数后清理，再由正常迁移登记 63。

新增真实数据库测试创建投票及关联名单，验证普通删除被拒、受控删除完成、另一委员会仍可读取。旧库重建、空库迁移和跨委员会删除 3 项定向通过；此前完整非数据库回归最终 88 文件 640 项全部通过。

## 2026-09-18 开发切换与实际界面验收

`quorum-dev` 从 schema 55 经限定委员会清理升级到 schema 63，删除 3 个旧委员会；事务验证账号、模板/成员、全局规则和系统设置保持。没有删除 Docker 卷或 Agent 本地目录。旧委员会的配对关系随委员会删除失效。88d2f5b 应用/Caddy 已重建并重新创建，HTTPS 使用现有 CA 验证，readiness 正常，API contract 3、schema 63；下列界面修复还需更新最终镜像。

实际浏览器通过 localhost:5173 Vite 代理连接已升级后端，创建中文和英文委员会，并逐一切换两种个人语言。席位、会期、默认文档和主发言名单使用委员会语言；按钮、状态、分页无障碍标签随个人语言改变。正文包含中英混合内容，完成点名后保存并刷新仍原样存在。错误后切换语言保留编辑草稿。自由磋商计时从 10:00 开始，切换到中文后继续，在 9:05 切回英文仍为 9:05。

第二入口 localhost:5174 与第一入口同时打开同一委员会，分别中文和英文，正文和默认标题一致，两者显示实时连接。此检查使用同一登录账号、独立 origin 的本地语言偏好，不是两个独立账号的权限验收。HTTPS 浏览器入口因浏览器不信任本地 CA 未绕过安全提示；HTTPS 服务另用受信 CA 的 curl 核验。

本轮修复：主发言名单导航使用快照名称；未修改自动标题/正文失焦不再因 null 与 undefined 差异写入版本；自定义模板成员可编辑双语名称；创建页展示缺失翻译的来源及成员；原生 Slint 在注册翻译回调后再设语言，修复首次英文启动部分标签为空。原生 ui-preview 的英文/中文小窗口已实际渲染，英文文件表头与工具栏恢复；它使用模拟数据，不代表真实配对传输。Rust 测试 1 项通过。
...
用户明确主发言名单菜单是固定功能入口：导航标签恢复跟随个人界面语言；名单页面内的名称继续按委员会内容语言显示。此前将导航标签归入资源内容的判断作废。导航测试核对同一链接切换中英文标签，路由及内容名称不变。
