...
规则包用于帮助主席团依照议事规则操作。当前仅启用一个内置包「北京学术标准 2021」；`Quorum Default` 仅保留为规则 schema 测试 fixture。用户仍可克隆并建立委员会规则版本。
...
| 项目 | 回退或创建时行为 |
| --- | --- |
| 规则包 | 新委员会默认选择「北京学术标准 2021」的最新已发布版本 |
| 委员会可见性 | 创建时必须确认，界面推荐 `PRIVATE` |
| 运作模式 | 当前内置包为 `CHAIR_OPERATED` |
...
具体发言时间、附议数、门槛、让渡和议事阶段等学术默认值属于规则包本身。包作者省略这些字段时才使用 Quorum 产品默认定义。
...
- 点名回答类型、顺序和重新点名建议；
- 当前唯一启用的内置规则「北京学术标准 2021」第 6 版仅提供“出席”和“缺席”。点名只决定出席状态；使用委员会模板时，模板给出表决权、否决权和不得弃权的初始值，之后在席位设置页面调整，手动新增席位也在该页面设置。规则包的点名回答不得授予或撤销这些属性；
