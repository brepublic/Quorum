...
状态：实施中。8.1 已落地 Owner 归档、全工作区只读约束和一致性流式导出；8.2 已落地精确名称确认、durable 删除 job、provider/Agent/staging 清理屏障和 fenced 数据库清除。账号资源处置、保留策略和运维恢复仍待后续小任务。
...
下一步继续阶段 8：实施账号资源处置与匿名化，再完成保留策略、运维状态和恢复说明。Firebase 移除必须等阶段 8 完成并通过当前环境可执行的验证后再开始。
