...
状态：实施中。8.1 已落地 Owner 归档、全工作区只读约束和一致性流式导出；8.2 已落地精确名称确认、durable 删除 job、provider/Agent/staging 清理屏障和 fenced 数据库清除；8.3 已落地禁用前置、资源原子转移和不可逆匿名化。保留策略和运维恢复仍待后续小任务。
...
下一步继续阶段 8：完成事件、Session、任务与日志保留策略，再实现运维状态、容量告警汇总和恢复说明。Firebase 移除必须等阶段 8 完成并通过当前环境可执行的验证后再开始。
