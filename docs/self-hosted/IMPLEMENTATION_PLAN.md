...
状态：实施中。8.1 已落地 Owner 归档、全工作区只读约束，以及议事记录、审计和文件 manifest 的一致性流式导出；永久删除、账号资源处置、保留策略和运维恢复仍待后续小任务。
...
下一步继续阶段 8：先完成委员会永久删除，再实施账号资源处置、保留策略、运维状态与恢复说明。Firebase 移除必须等阶段 8 完成并通过当前环境可执行的验证后再开始。
