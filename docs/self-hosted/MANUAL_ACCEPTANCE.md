...
- 操作步骤：完成首次管理员创建、管理员登录、创建普通账号、临时密码首次登录、只输入新密码与确认密码完成强制改密、退出、重置密码、禁用和 Session 撤销；从登录页匿名入口打开公开委员会列表，并尝试匿名打开私有委员会。
- 通过条件：强制改密复用刚完成临时密码验证的 Session，不再次询问或传输临时密码；自愿改密仍验证当前密码；旧 Session 在每项撤销操作后失效；普通账号看不到或不能调用管理功能；登录页匿名入口可列出并打开 PUBLIC 委员会，PRIVATE 委员会不泄露存在性；浏览器只请求同源身份和业务 API。
- 自动化覆盖情况：身份 service、HTTP、客户端和页面测试覆盖状态分支、强制改密不重传临时密码及匿名入口；真实 Cookie、TLS、浏览器导航和 PostgreSQL Session 轮换仍需人工确认。
- 当前状态：因无服务器延期。
...
- 操作步骤：打开默认国家模板并检查国家表；克隆默认模板；新建国家模板时先只输入名称，确认空模板创建后再逐项添加、修改和删除国家；选择标准国旗、Emoji 和 WebP 图片国旗；检查七个内置委员会模板及成员预览；从内置模板和手工国家模板分别创建委员会；创建、修改和删除账号级委员会模板；修改源模板并确认席位/国旗快照不变；创建、编辑和删除笔记及文本帖子；制造陈旧 revision 后刷新。
- 通过条件：默认国家模板可完整查看且只读，克隆得到带“副本/copy”名称的账号模板；空国家模板可持久化；同一委员会模板不能重复加入同一国家；七个内置委员会模板均可选且能创建委员会；委员会名称、议题、会议名称和规则版本写入正确列；账号模板互相隔离；模板删除冲突列出当前账号占用项；席位快照不随源模板变化；帖子权限符合 member/Chair/Owner 矩阵；删除后正文清空；409 后页面重新读取且不覆盖新版本。
- 自动化覆盖情况：契约、HTTP、前端和 PostgreSQL 集成测试覆盖空模板、内置模板、克隆、创建委员会字段与权限；当前环境未提供 `TEST_DATABASE_ADMIN_URL`，真实克隆/创建事务、WebP 选择、双浏览器交互和代理 Cookie 仍需人工确认。
- 当前状态：因无服务器延期。
...
- 操作步骤：开始会期和点名，确认只显示“出席”“缺席”；不按当前顺序直接点选任意席位并连续改答两次；同时提交同一 revision；撤销和重置；完成点名后再次更正一席；追加暂时离场、返回和缺席事件；把委员会暂停后重试议事命令。
...
### SH-MAN-522 主席电脑文件分享与代表门户

- 前置条件：真实 PostgreSQL 16、Caddy TLS 和当前生产构建；一个活动且采用主席代办模式的测试委员会；一个开放会期；至少三个席位，分别处于出席、暂离和缺席状态；已配对且作为活动存储的 Chair Agent；Chair、Owner、普通 member 和未登录浏览器；两个独立浏览器配置文件或设备；三份不含个人信息的测试文件。另准备可切换运作模式、归档委员会、断开 Agent 和清除浏览器站点数据的权限。
- 主席端预期行为：只有 Owner/Chair 且委员会为主席代办、活动存储为 `CHAIR_AGENT` 时，“文件”页面显示“分享”子页并以链接和二维码展示同一能力入口；不存在开放会期时不能开始分享。结束分享后旧链接和所有已发放代表凭据立即失效。附件子页使用居中表格卡片；主席可上传文件、修改待审核文件的公布名称，并在工作文件、指令草案、决议草案之间选择类型。批准后文件立即从待审核列表移入代表端已发布文件；驳回后不发布，并进入既有逻辑删除和 Agent 清理流程。
- 首次进入预期行为：未登录浏览器通过链接或二维码进入 `/delegate-files`，URL fragment 不发送到服务端请求。页面只列出当前开放会期中出席或暂离的活动席位，不列出缺席、停用、其他会期或其他委员会席位。选择席位后必须先出现“确认后不可更改代表团身份。”弹窗；取消不产生身份，确认后页面进入文件门户。同一浏览器在有效凭据存在时刷新、重新打开链接或尝试选择另一席位，仍保持原身份且不能重新选择。清除站点数据后凭据可消失，此限制不宣称抵抗主动规避或实现设备级认证。
- 代表门户预期行为：顶部第一个菜单项以粗体显示当前委员会名称且点击无效果；随后是“已发布文件”和“上传文件”；右侧只显示“实时”“离线”或“主席端故障”，不显示账户。已发布文件逐个以居中卡片内嵌表格展示主席指定文件名、提交国、文件类型、提交时间和公布时间；蓝色“下载 ↓”按钮下载正确字节、文件名和 MIME，未发布、已驳回、其他委员会文件及猜测 UUID 均不可下载。
- 上传预期行为：只有已绑定且当前仍出席或暂离的代表可上传；选择文件和文件类型后，“提交 ↑”在上传期间变为进度条。浏览器内容完整上传、大小/SHA-256 校验通过且已成功排入 Chair Agent host commit 后，显示绿色“✔提交成功，等待审核”；这表示提交已被接收，不表示主席已经批准或文件已经公布。Chair Agent 完成持久提交后文件进入待审核。重复请求不产生重复文件版本；身份过期、分享结束、席位变为缺席、排队失败、大小或 SHA-256 不匹配时明确失败，不把半文件或失败上传公布。
- 发布与 SSE 预期行为：主席批准代表上传文件后，所有仍连接的有效代表门户收到一次发布事件并刷新文件列表。横幅必须精确呈现“`国家名 代表 提交的 文件名 现已可用。`”，即国家名后、文件名前后各有一个空格；不得把主席或普通账户上传误报为代表门户发布。横幅在出现 60 秒后消失，手动关闭可提前消失，点击同一文件的下载按钮也立即消失；点击其他文件下载不应移除它。SSE 断线显示“离线”并按浏览器机制重连，Chair Agent 不健康显示“主席端故障”，恢复后回到“实时”；重连游标不得重复横幅或漏掉尚未反映在文件列表中的发布。
- 生命周期与权限预期行为：切换为代表自主、取消活动 Chair Agent binding 或归档委员会时，活动分享自动结束并撤销代表会话；委员会处于允许继续会议工作的暂停状态时不单独结束分享。旧页面在分享失效后的读取、上传、下载和 SSE 均失败关闭。Owner/Chair 管理权限不赋予普通 member 或仅有 `SYSTEM_ADMIN` 的账号；代表 cookie 不能访问委员会普通成员 API，普通登录 Session 也不能替代代表 cookie。代表凭据为 Secure、SameSite=Lax、HttpOnly，CSRF cookie 可读但必须与请求头一致；数据库、事件、审计、日志和 URL 查询不得保存明文 capability、代表凭据或 CSRF token。
...
- 当前状态：2026-08-28 隔离 `quorum-cache-acceptance` Compose 的 PostgreSQL/app/Caddy 已健康运行，schema 50/50，readiness 200，Caddy 已加载 `flush_interval -1`。管理员缓存状态、空明细、配置 revision 更新、陈旧 revision 409 和匿名明细 401 已经真实 HTTP 验证。经用户明确说明 `quorum-dev` 无真实且无需保留的数据后，补充完成真实业务账号、浏览器和 Linux Agent 连续验收：三档管理员页无整页横向溢出，390 px 文件表在容器内滚动；管理员可见安全元数据且无下载入口，恢复原角色后普通账号不能进入 `/operations`。既有配对 Agent 从无 capability 升级为 protocol 2 的 `SSE_WAKE`/`CACHE_REFILL`；代表测试文件经 `HOST_COMMIT_BLOB` 在约 0.6 秒内进入 `REVIEW_PINNED`，批准后转为 `READY`；降低发布额度后列表首项被淘汰为 `MISSING`，恢复额度并请求下载只创建一个 `FETCH_BLOB_TO_CACHE`，从任务创建到 Agent 完成约 0.57 秒，浏览器收到 attachment download。Caddy 重启导致 SSE 断开，Agent 记录失败并在约 3 秒后重连。完整仓库入口仍有与本功能无关的既有 fixture/超时失败；10 路并发、Agent 持续离线直至 30 秒兜底、源文件篡改及三类容量边界的真实故障注入仍以定向 PostgreSQL/runtime 自动化为证据，未冒充本次浏览器实测。
